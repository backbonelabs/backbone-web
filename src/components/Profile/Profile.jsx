...
const heightInches = heightValuesIn.map(val => (
  <Option
    key={val}
    value={val}
    label={`${Math.floor(val / 12)}ft ${val % 12}in`}
  />
));
...
class Profile extends Component {
  ...
  constructor(props) {
    ...
    this.onHeightUnitChange = this.onHeightUnitChange.bind(this);
    this.onWeightUnitChange = this.onWeightUnitChange.bind(this);
  ...
  onHeightUnitChange(evt) {
    ...
    if (evt.target.value !== this.state.heightUnitPreference) {
      ...
      if (this.state.heightUnitPreference === "1") {
        // if unit is inches
        stateChanges.height = Math.round(
          this.state.height * heightConversion
        ).toString();
      }

      if (this.state.heightUnitPreference === "2") {
        // if unit is cm
        stateChanges.height = Math.max(
          1,
          Math.round(this.state.height / heightConversion)
        ).toString();
      }
  ...
  onWeightUnitChange(evt) {
    ...
    if (evt.target.value !== this.state.weightUnitPreference) {
      ...
      if (this.state.weightUnitPreference === "1") {
        // if unit is LB
        stateChanges.weight = Math.ceil(
          this.state.weight * weightConversion
        ).toString();
      }

      if (this.state.weightUnitPreference === "2") {
        // if unit is kg
        stateChanges.weight = Math.round(
          this.state.weight / weightConversion
        ).toString();
      }
  ...
  onSave() {
    ...
    const weightInInches = weightUnitPreference === "1"
      ? weight
      : weight / weightConversion;
    const heightinInches = heightUnitPreference === "1"
      ? height
    ...
    if (
      ...
      formErrorState.dateError ||
      formErrorState.nicknameError
    ) {
              ...
              <div className="profile-container__selectfield">
                <div className="profile-container__leftside-selectfield">
                  <SelectField
                    ...
                    disabled={disableForm}
                  >
                    {this.state.heightUnitPreference === "1"
                      ? heightInches
                      : heightCentimeter}
                  </SelectField>
                </div>
              ...
              <div className="profile-container__selectfield">
                <div className="profile-container__leftside-selectfield">
                  <SelectField
                    ...
                    disabled={disableForm}
                  >
                    {this.state.weightUnitPreference === "1"
                      ? weightPounds
                      : weightkilograms}
                  </SelectField>
                </div>
